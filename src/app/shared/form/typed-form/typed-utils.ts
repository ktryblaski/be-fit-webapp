...
export type ControlsIn<T> = T extends {[key: string]: any}
  ? { [Key in keyof T]: AbstractTypedControl }
  : never;

export type InferredControlsIn<T> = T extends {[key: string]: any}
  ? { [Key in keyof T]: TypedFormControlType<T[Key]> }
  : never;
...
export function values$<T>(control: AbstractTypedControl<T>): Observable<T> {
  return defer<Observable<T>>(() => control.valueChanges.pipe(
    startWith<T, T>(control.value)
  ));
}
